...
const initOrderApi = system.InitOrderSystem

type initApi struct{}

// auto run
func init() {
	//global.GVA_LOG.Info("==== initApi:init")
	fmt.Println("==== initApi:init")
...
func (i *initApi) InitializeData(ctx context.Context) (next context.Context, err error) {
	global.GVA_LOG.Info("==== initApi:InitializeData")
	db, ok := ctx.Value("db").(*gorm.DB)
	if !ok {
		return ctx, system.ErrMissingDBContext
	}
	entities := []sysModel.SysApi{
		{ApiGroup: "jwt", Method: "POST", Path: "/jwt/jsonInBlacklist", Description: "wt加入黑名单(退出，必选)"},
	...
	if err := db.Create(&entities).Error; err != nil {
		global.GVA_LOG.Info("api 数据表初始化失败")

		return ctx, errors.Wrap(err, sysModel.SysApi{}.TableName()+"数据表初始化失败")
	}
	global.GVA_LOG.Info("api 插入数据成功")
